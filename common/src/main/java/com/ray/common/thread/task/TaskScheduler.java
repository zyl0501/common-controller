...
import static com.ray.common.util.OS.CPU_COUNT;
            ...
            if (oldTask != null) {
                ...
                if (dualPolicy == IGroupedTask.DISCARD_NEW) {//2.如果同一个组的相同任务已经被提交，则丢弃该任务
                    if (BuildConfig.DEBUG) {
                        Toasts.showShort(Common.ctx(), "任务被重复提交，新任务被丢弃，taskName=" + task.taskName());
                    }
                    return;
                } else if (dualPolicy == IGroupedTask.CANCEL_OLD) {
                    oldTask.stop();
                    if (BuildConfig.DEBUG) {
                        Toasts.showShort(Common.ctx(), "任务被重复提交，老任务被取消，taskName=" + task.taskName());
                    }
                } else if (dualPolicy == IGroupedTask.FORCE_SUBMIT) {
                    task.taskName = task.taskName + "_1";
                    if (BuildConfig.DEBUG) {
                        Toasts.showShort(Common.ctx(), "任务被重复提交，新任务被重命名，taskName=" + task.taskName());
                    }
                }
            }
