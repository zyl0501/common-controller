...
import com.ray.common.event.UIChangeEvent;
...
import static com.ray.common.thread.task.IPriorityTask.PRIOR_NORMAL;

/*package*/ final class TaskPriorityManager {
        ...
        switch (status) {
            case UIChangeEvent.STATUS_START://UI启动恢复被降级的任务
                for (AbstractTask<?> task : tasks) {
                    if (task.priority != PRIOR_NORMAL) {//只恢复被onStop事件降级的任务
