...
import com.bigkoo.pickerview.listener.OnTimeChangedListener;
...
public class TimePickerView extends BasePickerView implements View.OnClickListener {
  ...
  private int Color_Center_Text_Select = 0xFF1c8be4;
  private int Color_Center_Text_Nomal = 0xFF333333;
  ...
  public TimePickerView(Builder builder) {
    ...
    this.intervalTimeSelectListener = builder.intervalTimeSelectListener;
    ...
    this.Color_Center_Text_Nomal = builder.Color_Center_Text_Nomal;
    this.Color_Center_Text_Select = builder.Color_Center_Text_Select;
    this.Color_Background_Title = builder.Color_Background_Title;
    ...
    this.intervalAmount = builder.intervalAmount;
    this.isIntervalTime = builder.isIntervalTime;
  ...
  public static class Builder {
    ...
    private int Color_Center_Text_Nomal;//文字的初始颜色
    private int Color_Center_Text_Select;//文字的选中颜色
    ...
    private WheelView.DividerType dividerType;//分隔线类型
    // 条目间距倍数 默认1.6
    private float lineSpacingMultiplier = 3F;
    ...
    public Builder setCenterTextColor(int Color_Center_Title) {
      this.Color_Center_Text_Select = Color_Center_Title;
      return this;
    }

    public Builder setCenterTextColorNomal(int Color_Title_Nomal) {
      this.Color_Center_Text_Nomal = Color_Title_Nomal;
      return this;
    }
    ...
    public Builder setIntervalTime(boolean b) {
      isIntervalTime = b;
      return this;
    }

    public Builder setOnIntervalTimeSelectedListener(OnIntervalTimeSelectListener listener) {
      this.intervalTimeSelectListener = listener;
      return this;
    }

    public Builder setIntervalAmount(int amount) {
      intervalAmount = amount > 0 ? amount : 0;
      return this;
    }
  }
  ...
  private void initView(final Context context) {
    ...
    if (customListener == null) {
      ...
      llIntervalTime = (LinearLayout) findViewById(R.id.ll_intervalTime);
      tvLeftTime = (TextView) findViewById(R.id.tvLeftTime);
      tvRightTime = (TextView) findViewById(R.id.tvRightTime);
      ...
      tvLeftTime.setOnClickListener(new View.OnClickListener() {
        @Override
        public void onClick(View v) {
          onFocusLeft = true;
          onFocusRight = false;
          tvLeftTime.setTextColor(Color_Center_Text_Select);
          tvRightTime.setTextColor(Color_Center_Text_Nomal);

          String leftTime = tvLeftTime.getText().toString();
          SimpleDateFormat format = new SimpleDateFormat("yyyy-MM-dd");
          try {
            Date parseDate = format.parse(leftTime);
      ...
      tvRightTime.setOnClickListener(new View.OnClickListener() {
        @Override
        public void onClick(View v) {
          onFocusLeft = false;
          onFocusRight = true;

          tvLeftTime.setTextColor(Color_Center_Text_Nomal);
          tvRightTime.setTextColor(Color_Center_Text_Select);

          String rightTime = tvRightTime.getText().toString();
          SimpleDateFormat format = new SimpleDateFormat("yyyy-MM-dd");
          try {
            Date parseDate = format.parse(rightTime);
            Calendar calendar = Calendar.getInstance();
            calendar.setTime(parseDate);
            setDate(calendar);
          } catch (ParseException e) {
      ...
      if (isIntervalTime) {
        ...
        llIntervalTime.setVisibility(View.VISIBLE);
        Calendar calendar = Calendar.getInstance();
        calendar.add(Calendar.DAY_OF_MONTH, 3);
        SimpleDateFormat format = new SimpleDateFormat("yyyy-MM-dd");
        leftTime = format.format(calendar.getTime());
        tvLeftTime.setText(leftTime);

        calendar.add(Calendar.DAY_OF_MONTH, intervalAmount);
        rightTime = format.format(calendar.getTime());
        tvRightTime.setText(rightTime);
      } else {
        tvTitle.setVisibility(View.VISIBLE);
        llIntervalTime.setVisibility(View.GONE);
      }
      ...
      LinearLayout rv_top_bar = (LinearLayout) findViewById(R.id.rv_topbar);
    ...
    //时间发生改变的监听
    wheelTime.setOnTimeChangedListener(new OnTimeChangedListener() {
      @Override
      public void onTimeChanged() {
        String wheelTimeTime = wheelTime.getTime();
        String time = TextUtils.isEmpty(wheelTimeTime) ? "" : wheelTimeTime;
        String[] split = time.split(" ");
        if (split != null && split.length >= 2) {
          String s = split[0];
          if (isIntervalTime) {
            if (onFocusLeft) {
              tvLeftTime.setText(s);
              leftTime = s;
            }
            if (onFocusRight) {
              tvRightTime.setText(s);
              rightTime = s;
            }
          } else tvTitle.setText(s);
        }
      }
    });
  ...
  public void returnData() {
    ...
    if (null != intervalTimeSelectListener)
      intervalTimeSelectListener.onIntervalTimeSelect(leftTime, rightTime);
  }
  ...
  public interface OnIntervalTimeSelectListener {
    void onIntervalTimeSelect(String startTime, String endTime);
  }
